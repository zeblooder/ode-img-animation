...
def animate(config, generator, kp_detector, checkpoint, log_dir, dataset):
    ...
    for it, x in tqdm(enumerate(dataloader)):
        with torch.no_grad():
            ...
            source_frame = torch.tensor(resize(source_frame.squeeze(0).permute(1,2,0), (256, 256))[..., :3]).permute(2,0,1).unsqueeze(0)
            driving_video = torch.tensor([resize(frame, (256, 256))[..., :3] for frame in driving_video.squeeze(0).permute(1,2,3,0)]).permute(3,0,1,2).unsqueeze(0)
